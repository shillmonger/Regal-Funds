...
function InvestmentCard({ inv, index, onDelete, onRefresh }: { inv: Investment; index: number; onDelete: (id: string) => void; onRefresh: () => void }) {
  ...
  const isOutdated = () => {
    ...
    const lastAccruedAt = inv.lastAccruedAt ? new Date(inv.lastAccruedAt) : null;
    const durationDays = inv.durationDays || inv.duration;

    // Investment is outdated if:
    // 1. Status is Active (not completed)
    // 2. Current date is past the end date OR
    // 3. Days accrued is less than duration days AND last accrual was more than 2 days ago
    const isPastEndDate = now > endDate;
    const hasMissingDays = inv.daysAccrued < durationDays;
    const lastAccrualOld = lastAccruedAt ? (now.getTime() - lastAccruedAt.getTime()) > (2 * 24 * 60 * 60 * 1000) : false;

    return inv.status === 'Active' && (isPastEndDate || (hasMissingDays && lastAccrualOld));
  };
  ...
  // Calculate end date from approvedAt and duration
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/investments/${inv._id}`, {
        method: 'DELETE',
      });
  ...
  const handleResume = async () => {
    setIsResuming(true);
    try {
      const response = await fetch(`/api/investments/${inv._id}/resume`, {
        method: 'POST',
      });
  ...
  return (
    <div className="bg-white dark:bg-[#0f1623] border border-gray-200/80 dark:border-white/[0.06] rounded-2xl overflow-hidden flex flex-col hover:shadow-2xl hover:border-gray-300/80 dark:hover:border-white/[0.12] transition-all duration-500 group">
      {/* Header */}
      <div
      ...
      >
        ...
        <div className="text-right relative z-10">
          ...
          <div className="bg-[#448D96] text-white px-3 py-1 rounded-lg text-xs font-black ">
            {inv.roi}% ROI
          </div>
        </div>
      </div>
      ...
      <div className="p-4 md:p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="space-y-1">
            ...
            <p className="text-lg font-black text-sky-600 dark:text-sky-400 tracking-tighter">
          ...
          <div className="space-y-1 text-right">
            ...
            <p className="text-lg font-black tracking-tighter text-amber-600 dark:text-amber-400">
              {inv.daysAccrued}/{inv.durationDays}d
            </p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-end">
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-slate-600">
              Maturity Progress
            </span>
            <span className="text-xs font-black text-gray-900 dark:text-white">
              {Math.round((inv.daysAccrued / inv.durationDays) * 100)}%
            </span>
          </div>
          <div className="h-2 w-full bg-gray-100 dark:bg-white/[0.08] rounded-full overflow-hidden border border-gray-200 dark:border-white/[0.06]">
            <div
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="cursor-pointer px-3 bg-gray-200 dark:bg-white/[0.08] hover:bg-gray-300 dark:hover:bg-white/[0.12] text-gray-900 dark:text-white py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-[#0f1623] border border-gray-200/80 dark:border-white/[0.06] rounded-2xl p-6 max-w-md w-full shadow-2xl">
            ...
            {inv.status === "Active" ? (
              <div className="flex gap-3">
                <button
                  ...
                  disabled={isDeleting}
                  className="flex-1 bg-gray-100 dark:bg-white/[0.08] hover:bg-gray-200 dark:hover:bg-white/[0.12] text-gray-900 dark:text-white py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer"
                >
                  Cancel
                </button>
                {isOutdated() && (
                  <button
                    ...
                    className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {isResuming ? (
                      <>
                ...
                <button
                  ...
                  disabled={isDeleting}
                  className="flex-1 bg-red-600 hover:bg-red-500 text-white py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isDeleting ? (
                    <>
                      <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />
  ...
  const stats = [
    {
      ...
      value: `$${formatCompactNumber(investments.reduce((sum, inv) => sum + inv.amount, 0))}`,
      icon: Briefcase,
    ...
    },
    {
      ...
      value: `$${formatCompactNumber(investments.reduce((sum, inv) => sum + inv.earnings, 0))}`,
      icon: TrendingUp,
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50/50 dark:bg-[#080d17]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto pb-25 p-4 md:p-8">
          <div className="max-w-7xl mx-auto space-y-8 lg:space-y-9">
            {/* Page Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                {/* Tab Switcher */}
                <div className="inline-flex flex-1 sm:flex-none bg-gray-100 dark:bg-white/[0.08] p-1 rounded-xl border border-gray-200 dark:border-white/[0.06]">
                  {["all", "Active", "Completed"].map((tab) => (
                    <button
                      key={tab}
                      onClick={() => setActiveTab(tab as any)}
                      className={`cursor-pointer flex-1 sm:flex-none px-3 sm:px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all text-center ${activeTab === tab
                ...
                <button
                  ...
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-gray-200 dark:border-white/[0.06] bg-gray-100 dark:bg-white/[0.08] transition-colors hover:bg-gray-200 dark:hover:bg-white/[0.12] cursor-pointer"
                >
                  <TextAlignJustify className="h-4 w-4 text-gray-600 dark:text-slate-400" />
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {stats.map((stat, i) => (
                <div
                  key={i}
                  className={`bg-white dark:bg-[#0f1623] border border-gray-200/80 dark:border-white/[0.06] px-5 py-3 cursor-pointer rounded-2xl hover:border-gray-300/80 dark:hover:border-white/[0.12] transition-colors group ${stat.hoverBg} ${stat.hoverColor}`}
                >
                  {/* Icon container */}
                  <div
                  ...
                  <p className="text-2xl font-black tracking-tighter mt-2 text-gray-900 dark:text-white">
                    {stat.value}
                  </p>

                  <p className="text-[9px] font-black uppercase text-gray-400 dark:text-slate-600 tracking-widest">
                    {stat.label}
                  </p>
                </div>
              ))}
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="bg-white dark:bg-[#0f1623] border border-gray-200/80 dark:border-white/[0.06] rounded-2xl p-6 animate-pulse"
                  >
                    {/* Header */}
                    <div className="flex justify-between items-start mb-6">
                      <div className="space-y-2">
                        <div className="h-6 bg-gray-200 dark:bg-white/[0.08] rounded w-32" />
                        <div className="h-4 bg-gray-200 dark:bg-white/[0.08] rounded w-24" />
                      </div>
                      <div className="w-2 h-2 bg-gray-200 dark:bg-white/[0.08] rounded-full" />
                    </div>

                    {/* Stats */}
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="space-y-2">
                        <div className="h-3 bg-gray-200 dark:bg-white/[0.08] rounded w-16" />
                        <div className="h-8 bg-gray-200 dark:bg-white/[0.08] rounded w-20" />
                      </div>
                      <div className="space-y-2">
                        <div className="h-3 bg-gray-200 dark:bg-white/[0.08] rounded w-16" />
                        <div className="h-8 bg-gray-200 dark:bg-white/[0.08] rounded w-20" />
                    ...
                    {/* Progress */}
                    <div className="space-y-2">
                      <div className="h-3 bg-gray-200 dark:bg-white/[0.08] rounded w-24" />
                      <div className="h-2 bg-gray-200 dark:bg-white/[0.08] rounded-full w-full" />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {filteredInvestments.map((inv, index) => (
                  <InvestmentCard key={inv._id.toString()} inv={inv} index={index} onDelete={handleDeleteInvestment} onRefresh={fetchInvestments} />
                ))}
              </div>
            ) : (
              <div className="text-center py-20 border border-dashed border-gray-200 dark:border-white/[0.06] rounded-2xl">
                <BarChart2 className="w-12 h-12 text-gray-400 dark:text-slate-600 mx-auto mb-4 opacity-20" />
