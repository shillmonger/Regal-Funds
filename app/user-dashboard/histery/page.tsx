    ...
    <div className="flex h-screen overflow-hidden bg-gray-50 dark:bg-gray-950">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-6 lg:p-8 pb-20 md:pb-8 mb-[50px] md:mb-0">
          ...
          {/* Statistics Cards (Omitted for brevity, assuming correct) */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-8">
            ...
            <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
              <CardContent className="p-6">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-1">Total Deposits</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            ...
            <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
              <CardContent className="p-6">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-1">Withdrawals</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            ...
            <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
              <CardContent className="p-6">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm mb-1">Total Earnings</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
          ...
          <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 mb-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {/* 1. Transaction Type Filter - ALREADY SHADCN SELECT */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Transaction Type
                  </label>
                  <Select value={filterType} onValueChange={setFilterType}>
                    <SelectTrigger className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg text-gray-900 dark:text-gray-100 focus:ring-emerald-500">
                ...
                {/* 2. Status Filter - UPDATED to use Shadcn Select */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Status
                  </label>
                  <Select value={filterStatus} onValueChange={setFilterStatus}>
                    ...
                    <SelectContent>
                      ...
                      {Object.entries(statusTypes).map(([key, config]) => (
                        <SelectItem key={key} value={key}>
                          <div className="flex items-center">
                            <config.icon className={`w-4 h-4 mr-2 ${config.color}`} />
                            {config.label}
                ...
                {/* 3. Date Filter - UPDATED to use Shadcn Select */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Date Range
                  </label>
              ...
              {/* Active Filters Display (Omitted for brevity, assuming correct) */}
              {(filterType !== "all" || filterStatus !== "all" || dateFilter !== "all" || searchQuery) && (
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Active filters:</span>
                  {searchQuery && (
                    <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 text-xs font-medium rounded-full">
                      Search: {searchQuery}
                    </span>
                  )}
                  {filterType !== "all" && (
                    <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-xs font-medium rounded-full">
                      Type: {transactionTypes[filterType as keyof typeof transactionTypes]?.label}
                    </span>
                  )}
                  {filterStatus !== "all" && (
                    <span className="px-3 py-1 bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 text-xs font-medium rounded-full">
                      Status: {statusTypes[filterStatus as keyof typeof statusTypes]?.label}
                    </span>
                  )}
                  {dateFilter !== "all" && (
                    <span className="px-3 py-1 bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-300 text-xs font-medium rounded-full">
                      Date: {dateRangeOptions.find(o => o.value === dateFilter)?.label}
                    </span>
                  )}
                  <button
                    onClick={() => {
                    ...
                    className="px-3 py-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-full hover:bg-gray-300 dark:hover:bg-gray-600 transition"
                  >
                    Clear All
                  </button>
                </div>
          ...
          {/* Transaction List (Omitted for brevity, assuming correct) */}
          <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
            <CardHeader>
              <CardTitle className="text-gray-900 dark:text-gray-100">
